...
-- Fix: PivotTo ke NPC dulu sebelum FireServer (proximity check bypass)
...
local function getGroundedCFrameNear(targetCFrame, distanceBack)
    distanceBack = distanceBack or 4
    ...
    if result then
        ...
        log("Raycast hit: " .. result.Instance:GetFullName() .. " Y=" .. tostring(math.floor(result.Position.Y)))
    else
...
-- Teleport client-side saja (PivotTo)
local function teleportToHoneySeedNPC()
    ...
    local finalCFrame = getGroundedCFrameNear(targetCFrame, 4)
    pcall(function() character:PivotTo(finalCFrame) end)

    task.wait(0.25)

    local after = root.Position
    log("Client PivotTo | Before: " .. shortPos(before) .. " | After: " .. shortPos(after))

    return true, "Client PivotTo. Pos: " .. shortPos(after)
end

-- PivotTo ke NPC dulu, tunggu server acknowledge, baru FireServer
local function firePlayerTeleportTriggered()
    -- Step 1: Cari NPC dan CFrame-nya
    local npc, npcErr = getHoneySeedNPC()
    if not npc then return false, "NPC not found: " .. tostring(npcErr) end

    local npcCFrame = getInstanceCFrame(npc)
    if not npcCFrame then return false, "Could not get NPC CFrame" end

    -- Step 2: PivotTo tepat di samping NPC (2 studs)
    ...
    local nearCFrame = getGroundedCFrameNear(npcCFrame, 2) -- sangat dekat NPC
    pcall(function() character:PivotTo(nearCFrame) end)

    log("PivotTo NPC done. Pos: " .. shortPos(root.Position))
    log("Dist to NPC: " .. tostring(math.floor((root.Position - npcCFrame.Position).Magnitude)))

    -- Step 3: Tunggu server acknowledge posisi baru
    task.wait(0.6)
    ...
    -- Step 5: Tunggu server proses
    task.wait(1.5)

    local distToNPC = (root.Position - npcCFrame.Position).Magnitude
    log("Final dist to NPC: " .. tostring(math.floor(distToNPC)))

    return true, "Done. Pos: " .. shortPos(root.Position)
end
...
-- AUTO: PivotTo + FireServer + buka UI sekaligus
local function autoHoneySeed()
    local npc, npcErr = getHoneySeedNPC()
    if not npc then return false, "NPC not found: " .. tostring(npcErr) end
    ...
    -- Step 1: PivotTo dekat NPC
    root.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
    root.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
    local nearCFrame = getGroundedCFrameNear(npcCFrame, 2)
    pcall(function() character:PivotTo(nearCFrame) end)
    log("[AUTO] PivotTo done. Dist: " .. tostring(math.floor((root.Position - npcCFrame.Position).Magnitude)))
    ...
    if not fireOk then
        return false, "FireServer failed: " .. tostring(fireErr)
    end

    log("[AUTO] FireServer sent")
    task.wait(1.5)

    -- Step 3: Buka UI shop
    local uiOk, uiMsg = openHoneySeedShopUI()
    if not uiOk then
        return true, "Teleport done, UI failed: " .. uiMsg
    end

    return true, "AUTO done. Shop opened. Pos: " .. shortPos(root.Position)
end
...
-- Tombol AUTO paling atas (highlight hijau terang)
local AutoBtn       = makeButton("AutoHoneySeed",        "⚡ AUTO: Teleport + Fire + Open Shop", 186, Color3.fromRGB(20, 80, 20))
stroke(AutoBtn, Color3.fromRGB(57, 255, 20), 2, 0)

local TeleportBtn   = makeButton("TeleportHoneySeedNPC", "1. Client PivotTo NPC",               230, Color3.fromRGB(35, 55, 35))
local FireRemoteBtn = makeButton("FireRemote",           "2. PivotTo + FireServer (fix proxim)", 274, Color3.fromRGB(35, 35, 65))
local OpenShopBtn   = makeButton("OpenHoneySeedShop",    "3. Open Honey Seed Shop UI",           318, Color3.fromRGB(55, 45, 35))
local PosBtn        = makeButton("PrintPosition",        "Print Position",                       362, Color3.fromRGB(35, 45, 55))
...
AutoBtn.MouseButton1Click:Connect(function()
    setStatus("AUTO: PivotTo → FireServer → Open UI...", Color3.fromRGB(255, 220, 80))
    local ok, msg = autoHoneySeed()
    if ok then
...
TeleportBtn.MouseButton1Click:Connect(function()
    setStatus("Client PivotTo NPC...", Color3.fromRGB(255, 220, 80))
...
FireRemoteBtn.MouseButton1Click:Connect(function()
    setStatus("PivotTo NPC → FireServer...", Color3.fromRGB(255, 220, 80))
